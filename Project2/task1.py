...
def check_output_format(faces, img, img_name):
    ...
    for i, face in enumerate(faces):
        ...
        if face[0] >= img.shape[1] or face[1] >= img.shape[0] or face[0] + face[2] >= img.shape[1] or face[1] + face[3] >= img.shape[0]:
            ...
            #print("face[0] >= img.shape[1]",face[0] >= img.shape[1] , face[0] , ">= ", img.shape[1] )
            #print("face[1] >= img.shape[0]",face[1] >= img.shape[0] , face[1] , ">= ", img.shape[0])
            print("face[0] + face[2] >= img.shape[1]", face[0] + face[2] >= img.shape[1],"face[0] ",face[0]," face[2]",face[2], img.shape[1])
            
            cv2.imshow("warning img",img)

            cv2.waitKey(0)\
            #print(" face[1] + face[3] >= img.shape[0]", face[1] + face[3] >= img.shape[0],face[1] , face[3])
